  ...
  "return Right when one of the multiple appeal submissions fails because of a fault (logging a PD)" in {
    ...
    withCaptureOfLoggingFrom(logger) {
      logs => {
        ...
        logs.exists(_.getMessage.contains("LPP1 appeal was not submitted successfully, Reason given An issue occurred whilst appealing a penalty with error: Connection reset")) shouldBe true
