...
import connectors.httpParsers.{PayNowHttpParser, UnexpectedFailure}

import models.payApi.PayNowResponseModel
...
//scalastyle:off
class PayNowServiceSpec extends SpecBase with LogCapturing {

  implicit val hc: HeaderCarrier = HeaderCarrier()
  implicit val ec: ExecutionContext = ExecutionContext.Implicits.global
  val mockPayNowConnector: PayNowConnector = mock(classOf[PayNowConnector])
  val chargeReference="123456789"
  val vatAmount = 123
  val dueDate = LocalDate.now()

  val service = new PayNowService(mockPayNowConnector, mockAppConfig)

  val timeToPayResponseModel: PayNowResponseModel = PayNowResponseModel("592d4a09cdc8e04b00021459","http://localhost:1111/test")
  ...
  val PayNowResponse: HttpResponse = HttpResponse.apply(Status.CREATED, timeToPayResponseJson.toString())

  "retrieveRedirectUrl" should {
    "return a url when the call is successful" in {
      when(mockPayNowConnector.setupJourney(any())(any(), any()))
        .thenReturn(Future.successful(PayNowHttpParser.PayNowResponseReads.read("", "", PayNowResponse)))
