...
class HasBusinessAskedHMRCToCancelRegistrationControllerSpec extends SpecBase {
  ...
  "onSubmit" when {
    "the user is authorised" must {
      "return 303 (SEE_OTHER) adding the key to the session when the body is correct " +
        ...
        status(result) shouldBe SEE_OTHER
        answerCaptor.getValue.data.decryptedValue shouldBe correctUserAnswers ++ Json.obj(SessionKeys.hasBusinessAskedHMRCToCancelRegistration -> "yes")
      }

      "return 303 (SEE_OTHER) routing when a valid option is selected - removing the next page answer if the user answers 'no'" in new Setup(
        AuthTestModels.successfulAuthResult, Json.obj(SessionKeys.hasHMRCConfirmedRegistrationCancellation -> "yes")
      ) {
        val answerCaptor: ArgumentCaptor[UserAnswers] = ArgumentCaptor.forClass(classOf[UserAnswers])
        when(mockSessionService.updateAnswers(answerCaptor.capture()))
          .thenReturn(Future.successful(true))
        ...
        status(result) shouldBe SEE_OTHER
        answerCaptor.getValue.data.decryptedValue shouldBe {
          correctUserAnswers -
            SessionKeys.hasHMRCConfirmedRegistrationCancellation ++
            Json.obj(SessionKeys.hasBusinessAskedHMRCToCancelRegistration -> "no")
        }
      }
