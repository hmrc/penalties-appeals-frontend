...
object UploadFormTemplateRequest {
  implicit val formats: Format[UploadFormTemplateRequest] = Json.format[UploadFormTemplateRequest]
}
  ...
  implicit val jsonReadsForModel: Reads[UpscanInitiateResponseModel] = new Reads[UpscanInitiateResponseModel] {
    override def reads(json: JsValue): JsResult[UpscanInitiateResponseModel] = {
      for {
        ...
        fieldsAndHref <- (json \ "uploadRequest").validate[UploadFormTemplateRequest](UploadFormTemplateRequest.formats)
      } yield {
