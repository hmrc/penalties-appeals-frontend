...
class AppealByLetterKickOutController @Inject()(page: AppealByLetterKickOutPage)(
  ...
  val pageMode: Mode => PageMode = (mode: Mode) => PageMode(AppealByLetterKickOutPage, mode)

  def onPageLoad(): Action[AnyContent] = (authorise andThen dataRetrieval andThen dataRequired) {
    implicit request => Ok(page(pageMode(NormalMode)))
  }
}
